...
old_points <- thepoints

# project the points for correct area measurements
#thepoints <- simProjWiz(thepoints)
#old_points <- simProjWiz(thepoints)
#old_points_geog <- st_transform(old_points, 4326)
...
# new recs
files <- list.files("data/Trachelophorus_giraffa",
                    pattern = "new_records.*\\.rds$",
                    full.names = TRUE)

df_list <- lapply(files, readRDS)
new_recs <- do.call(rbind, df_list)

# run cleaning protocols
#flag_errors <- LCr::flag_occs(new_recs)
#clean_points <- LCr::clean_occs(flag_errors$flagged_data)

clean_points <- new_recs %>%
  filter(country == "Madagascar")
...
new_points <- thepoints

# project the points for correct area measurements
#new_points <- simProjWiz(thepoints)
#new_points_geog <- st_transform(new_points, 4326)
...
m <- leaflet() %>%
  ...
  addCircleMarkers(
    ...
    group = "old points"     # Layer group for toggling
  ) %>%
  ...
  addCircleMarkers(
    ...
    fillOpacity = 0.8,       # Fill transparency
    group = "new points"     # Layer group for toggling
  ) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("new points", "old points"),
